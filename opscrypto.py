    ...
    if args["decrypt"]:
        ...
        for child in root:
            if child.tag == "SAHARA":
                for item in child:
                    if item.tag == "File":
                        # Get the filename from "Path" attribute
                        wfilename = item.attrib.get("Path", "")
                        # Only process files that contain "boot"
                        if "boot" not in wfilename.lower():
                            continue
                ...
                for item in child:
                    if "filename" in item.attrib:
                        wfilename = item.attrib.get("filename", "")
                        if "boot" not in wfilename.lower():
                            continue
                        start = int(item.attrib["FileOffsetInSrc"]) * 0x200
                        slength = int(item.attrib["SizeInSectorInSrc"]) * 0x200
                        ...
                        if sha256 != csha256:
                            print("Sha256 fail for", wfilename)
                    else:
                        for subitem in item:
                            if "filename" in subitem.attrib:
                                wfilename = subitem.attrib.get("filename", "")
                                if "boot" not in wfilename.lower():
                                    continue
